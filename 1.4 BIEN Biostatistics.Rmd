...
```{r}
Tomato_DBH <- BIEN_trait_traitbygenus(trait = "diameter at breast height (1.3 m)", genus = "Solanum")
head(Tomato_DBH)
tail(Tomato_DBH)
str(Tomato_DBH)
```
...
```{r}
# List variable names from Tomato_DBH data frame
names(Tomato_DBH)
```

```{r}
Tomato_DBH_small = Tomato_DBH[c(1:5)]
Tomato_DBH_small
Tomato_DBH_small = as.data.frame(Tomato_DBH_small)
class(Tomato_DBH_small)
head(Tomato_DBH_small)
class(Tomato_DBH_small$trait_value)
DBH = as.numeric(Tomato_DBH_small$trait_value)
class(DBH)
```
...
library(lattice)
head(Tomato_DBH_small)
boxplot(DBH~scrubbed_species_binomial, data=Tomato_DBH_small)
```
...
```{r}
mean(DBH, data=Tomato_DBH_small)
...
```{r}
Prunicana_DBH <- BIEN_trait_traitbyspecies(trait = "diameter at breast height (1.3 m)", species = "Prunus africana")
head(Prunicana_DBH)
tail(Prunicana_DBH)
str(Prunicana_DBH)
```
...
```{r}
Prunicana_DBH_small = Prunicana_DBH[c(1:5)]
Prunicana_DBH_small
Prunicana_DBH_small = as.data.frame(Prunicana_DBH_small)
class(Prunicana_DBH_small)
head(Prunicana_DBH_small)
class(Prunicana_DBH_small$trait_value)
DBHa = as.numeric(Prunicana_DBH_small$trait_value)
class(DBHa)
```
...
```{r}
Samericanum_DBH <- BIEN_trait_traitbyspecies(trait = "diameter at breast height (1.3 m)", species = "Solanum americanum")
head(Samericanum_DBH)
tail(Samericanum_DBH)
str(Samericanum_DBH)
```


```{r}
Parmeniaca_DBH_small = Parmeniaca_DBH[c(1:5)]
Parmeniaca_DBH_small
Parmeniaca_DBH_small = as.data.frame(Parmeniaca_DBH_small)
class(Parmeniaca_DBH_small)
head(Parmeniaca_DBH_small)
DBHh = as.numeric(Parmeniaca_DBH_small$trait_value)
class(Parmeniaca_DBH_small$trait_value)
class(DBHh)
```
...
```{r}
library(lattice)
Solanum2<-rbind(Parmeniaca_DBH_small, Parmeniaca_DBH_small)
head(Solanum2)
tail(Solanum2)
class(Solanum2$trait_value)
DBHboth = as.numeric(Solanum2$trait_value)
class(DBHboth)
```
...
```{r}
SolanumBoth<-cbind(Solanum2, DBHboth)
head(SolanumBoth)
tail(SolanumBoth)
```
...
```{r}
boxplot(DBHboth~scrubbed_species_binomial, data=SolanumBoth)
```
...
library(doBy)
summaryBy(DBH~scrubbed_species_binomial, data=Tomato_DBH_small, FUN=c(mean, sd))
summaryBy(WPH~scrubbed_species_binomial, data=Tomato_WPH_small, FUN=c(mean, sd))
```

```{r}
(unique(Tomato_DBH_small$scrubbed_species_binomial))
```
...
# Population Mean Between Two Independent Samples
# http://www.r-tutor.com/elementary-statistics/inference-about-two-populations/population-mean-between-two-independent-samples
ttest_result1 = t.test(x=Tomato_WPH_small$scrubbed_species_binomial, y=Tomato_WPH_small$trait_value, paired=FALSE, alternative="two.sided")
ttest_result1
```
